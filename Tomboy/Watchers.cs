	...
	public class NoteLinkWatcher : NotePlugin
	{
		...
		void OnNoteRenamed (Note renamed, string old_title)
		{
			...
			foreach (TextRange range in enumerator) {
				...
				Gtk.TextIter replaceStart = range.Start;
				Gtk.TextIter replaceEnd = range.End;

				Buffer.Delete (ref replaceStart, ref replaceEnd);
				Buffer.InsertWithTags (ref replaceStart, renamed.Title, link_tag);
			}
		}
